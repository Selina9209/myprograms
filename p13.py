...
print(min(c_list)) # 1
print(max(c_list)) # 9 
    #min and max are both linear search
...
c = a + b # creates a new list of a and b joined, do not manipulate a or b
a.extend(b) # this mutates a to give the contents of b, a got maipulated to get b
a = [3, 1, 4]
for item in b:
    a.append(item)

# Reverse a list (two ways)
# Create a copt of a list (two ways)
# Compare lists for equality
# Determine if an item exists within a list
# Count how often an item occurs within a list
# Convert a string to a list
# Sort a list
# Sort two lists where the index are attached to each other based on one of the list